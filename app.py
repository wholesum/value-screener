...
# ------------------------------------------------------------
# CACHE & HELPERS (with long history + weekly resampling)
# ------------------------------------------------------------
historical_cache = {}

def get_historical(ticker, period='10y', interval='1wk'):
    """
    Fetch max available data (10 years) and resample to weekly.
    Returns a pandas Series of weekly closing prices.
    """
    key = f"{ticker}_{period}_{interval}"
    if key in historical_cache:
        return historical_cache[key]
    try:
        # Use period='max' to get as much as possible, but limit to 10y for memory
        ...
        # Resample to weekly (last trading day of week)
...
# ------------------------------------------------------------
# ENDPOINTS (with full error handling)
# ------------------------------------------------------------
